...
public class StrengCardAction extends AbstractAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try{
			...
			if(!role.getFreshStep()[3].equals("17")){
				RoleManager roleManager = (RoleManager)HttpSpringUtils.getBean("roleManager");
				roleManager.updateFreshStep(3, 17, role.getRoleId());
			}
			result = userCardManager.StrengCard(new Long(userCardId), userCardIds);
			if(result==null){
				returnType.setMsg("强化失败！");
				returnType.setStatus(0);
				super.printReturnType2Response(response, returnType);
