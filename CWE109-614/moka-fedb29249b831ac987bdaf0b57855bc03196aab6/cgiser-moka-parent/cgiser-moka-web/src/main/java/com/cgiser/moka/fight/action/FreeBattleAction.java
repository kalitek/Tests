...
public class FreeBattleAction extends AbstractAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try{
			...
			if(this.isWin(aPlayer, dPlayer)&&fightResult.getWin()==0&&RoleStatusScanThread.freeFightRoom.containsKey(fightResult.getBattleId())){
				RoleStatusScanThread.freeFightRoom.remove(fightResult.getBattleId());
				...
				if(type==1||type==0){
					roleManager.updateRoleStatus(dPlayer.getNickName(), 0);
				}
			}
			//初始生成战斗模型后通知挑战者开始战斗
